...
function main(paths) {
  ...
  combos.sort(function (a, b) {
    var aa = getRange(paths[a[0][0]].pathPoints[a[0][1] ? paths[a[0][0]].pathPoints.length - 1 : 0], paths[a[1][0]].pathPoints[a[1][1] ? 0 : paths[a[1][0]].pathPoints.length - 1]);
    var bb = getRange(paths[b[0][0]].pathPoints[b[0][1] ? paths[b[0][0]].pathPoints.length - 1 : 0], paths[b[1][0]].pathPoints[b[1][1] ? 0 : paths[b[1][0]].pathPoints.length - 1]);
    if (aa < bb) return -1;
    if (aa > bb) return 1;
    return 0;
  });
...
function smoothing(points, index) {
  ...
  // 2点間の距離が0なら1点にする
  if (toRound(getRange(current, previous)) === 0) {
