...
function main(paths) {
  ...
  while (route.length < paths.length) {
    ...
    for (var i = 0; i < combos.length; i++) {
      ...
      // ルート格納
      if (route[0][0] === former[0] && route[0][1] === !former[1]) {
        route.unshift(latter);
        route[0][1] = !latter[1];
      } else if (route[0][0] === latter[0] && route[0][1] === latter[1]) {
        route.unshift(former);
      } else if (route[route.length - 1][0] === former[0] && route[route.length - 1][1] === former[1]) {
        route.push(latter);
      } else if (route[route.length - 1][0] === latter[0] && route[route.length - 1][1] === !latter[1]) {
        route.push(former);
        route[route.length - 1][1] = !former[1];
      }
      combos.splice(i, 1);
      break;
    }
